...
notify_tracker = {
}  # { classNumber: { lastSent: timestamp, interval: hours, notificationCount: int } }
...
def fetch_class_data(url, class_name):
    ...
    for item in data.get('classes', []):
        ...
        if class_number in WHITELIST:
            ...
            tracker = notify_tracker.get(class_number, {
                'lastSent': 0,
                'interval': 1,
                'notificationCount': 0
            })
            ...
            next_send = tracker[
                'lastSent'] + tracker['interval'] * 60 * 60 * 1000

            if enrolled_num < total_seats_num:
                if tracker['notificationCount'] >= MAX_NOTIFICATIONS_PER_CLASS:
                    print(
                        f"Max notifications ({MAX_NOTIFICATIONS_PER_CLASS}) reached for class {class_number} ({title}). Skipping."
                    )
                elif now >= next_send:
                    ...
                    print(
                        f"Next notification for class {class_number} in {tracker['interval']} hour(s). ({tracker['notificationCount'] + 1}/{MAX_NOTIFICATIONS_PER_CLASS})\n"
                    )
                    ...
                    print(
                        f"Skipping notification for class {class_number} ({title}), next notification in {mins_left} min(s). ({tracker['notificationCount']}/{MAX_NOTIFICATIONS_PER_CLASS})"
                    )
            else:
                if notify_tracker.get(class_number,
                                      {}).get('notificationCount', 0) > 0:
                    print(
                        f"Seats closed for class {class_number} ({title}), resetting notification count."
                    )
                    notify_tracker[class_number] = {
                        'lastSent': 0,
                        'interval': 1,
                    ...
                    print(
                        f"No open seats: ({class_number}) {name}, {title}, Instructor: {instructor}, Location: {location}, Seats: {enrolled} of {total_seats}\n"
                    )
